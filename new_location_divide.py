...
def predict(src):
    if isinstance(src, np.ndarray):
        ...
        with open(src, 'rb') as f:
            rawImage = cv2.imdecode(np.array(bytearray(f.read()), dtype=np.uint8), cv2.IMREAD_COLOR)
    ...
    # 以上为车牌定位
    # 以下为识别车牌中的字符
    # 变成要返回的列表包含状态码、原车牌、车牌颜色、灰度化后的车牌、预处理的图像
    predict_result = [0]
    roi = None
    card_color = None
    for i, color in enumerate(colors):
        if color in ("blue", "yellow", "green"):
            ...
            # 添加车牌图像到返回列表
            predict_result.append(card_img)
            ...
            if color == "green":
                # print("绿牌！")
                predict_result.append('green')
            elif color == "blue":
                # print("蓝牌！")
                predict_result.append('blue')
            elif color == "yellow":
                # print("黄牌！")
                predict_result.append('yellow')

            # 添加灰度化车牌图像到返回列表
            predict_result.append(gray_img)
            # 改变状态码到返回列表
            predict_result[0] = 1
    ...
    # 添加预处理的图像到返回列表
    # cv2.imshow("wie", rawImage)
    # cv2.waitKey(0)
    predict_result.append(rawImage)
